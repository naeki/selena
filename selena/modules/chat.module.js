  ...
  .keys(chatMessage)
    .waitForVisible("//*[@role='messageInput'][contains(text(),'" + chatMessage + "')]", TIMEOUT)
      .then(
        ...
        function(e){selena.regActionResult("Cообщение " + chatMessage + " не введено в инпут", "\n", e.message, 0)}
      )
  .keys(["Escape"])
  ...
  .taskCardOpen(taskName)
    .waitForVisible("[role='draft']", TIMEOUT)
      .then(
        ...
        function(e){selena.regActionResult("Проверка во втором табе: Индикатор draft не появился в чате карточки " + taskName + "", "\n", e.message, 0)}
      )
  .keys(["Escape"])
  ...
  .keys(["Enter"])
    .waitForVisible("//*[@role='messageInput'][contains(text(),'" + chatMessage + "')]", TIMEOUT, true)
      .then(
        ...
        function(e){selena.regActionResult("Cообщение " + chatMessage + " не пропало из инпута", "\n", e.message, 0)}
      )
  .keys(["Escape"])
  ...
  .taskCardOpen(taskName)
    .waitForVisible("//*[@role='messageHistory' and contains(text(),'" + chatMessage + "')]", TIMEOUT)
      .then(
        ...
        function(e){selena.regActionResult("Проверка во втором табе: Сообщение " + chatMessage + " появилось в истории карточки " + taskName + "", "\n", e.message, 0)}
      )
  .keys(["Escape"])
  .switchTabAndCallback(tabMap.first)
...
testModule.tests.chatChildArchiveExtract = {
    ...
    return this
        ...
        .waitForVisible("[role='archive']", TIMEOUT)
            .then(
                ...
                function(e){selena.regActionResult("Кнопка архивации не доступна", "\n", e.message, 0)}
            )
    .click("[role='archive']")
        .waitForVisible("[role='extract']", TIMEOUT)
            .then(
                function(){selena.regActionResult("Клик на Archive. Cменилась на Extract", 1)},
                function(e){selena.regActionResult("Клик на Archive. Не сменилась на Extract", "\n", e.message, 0)}
            )
    ...
    .sphereChatOpenCurrent()
        .waitForExist("//*[@role='task' and contains(@class,'task_archived')]//*[@role='title'][contains(text(),'" + taskName + "')]", TIMEOUT)
            .then(
                ...
                function(e){selena.regActionResult("Проверка в чате сферы: Карточка " + taskName + " не заархивировалась", "\n", e.message, 0)}
            )
    ...
    .click("[role='extract']")
        .waitForVisible("[role='archive']", TIMEOUT)
            .then(
                ...
                function(e){selena.regActionResult("Клик на Extract. Не сменилась на Archive", "\n", e.message, 0)}
            )
    ...
    .waitForExist("//*[@role='task' and contains(@class,'task_archived')]//*[@role='title'][contains(text(),'" + taskName + "')]", TIMEOUT, true)
            .then(
                ...
                function(e){selena.regActionResult("Проверка в чате сферы: Карточка " + taskName + " не разархивировалась", "\n", e.message, 0)}
            )
    .keys(["Escape"])
    ...
    return this
        ...
        .waitForVisible("[role='delete']", TIMEOUT)
            .then(
                ...
                function(e){selena.regActionResult("Кнопка удаления не доступна", "\n", e.message, 0)}
            )
    .click("[role='delete']")
        .waitForVisible("[role='recover']", TIMEOUT)
            .then(
                function(){selena.regActionResult("Клик на Delete. Cменилась на Recover", 1)},
                function(e){selena.regActionResult("Клик на Delete. Не сменилась на Recover", "\n", e.message, 0)}
            )
    ...
    .sphereChatOpenCurrent()
        .waitForExist("//*[@role='task']//*[@role='title'][contains(text(),'" + taskName + "')]", TIMEOUT, true)
            .then(
                ...
                function(e){selena.regActionResult("Проверка в чате сферы: Карточка " + taskName + " не удалилась", "\n", e.message, 0)}
            )
    ...
    .click("[role='recover']")
        .waitForVisible("[role='delete']", TIMEOUT)
            .then(
                ...
                function(e){selena.regActionResult("Клик на Recover. Не сменилась на Delete", "\n", e.message, 0)}
            )
    ...
    .switchTabAndCallback(tabMap.second)
        .waitForExist("//*[@role='task']//*[@role='title'][contains(text(),'" + taskName + "')]", TIMEOUT)
            .then(
                ...
                function(e){selena.regActionResult("Проверка в чате сферы: Карточка " + taskName + " не восстановилась", "\n", e.message, 0)}
            )
    ...
    .keys(["Enter"])
        .waitForVisible("//*[@role='messageHistory' and contains(text(),'" + chatMessageBefore + "')]", TIMEOUT)
            .then(
                ...
                function(e){selena.regActionResult("Сообщение " + chatMessageBefore + " не появилось в истории карточки " + taskName + "", "\n", e.message, 0)}
            )
    ...
    .click("//*[@role='dropdown']//*[@title='Edit']")
        .waitForVisible("//*[@contenteditable='true' and contains(text(),'" + chatMessageBefore + "')]", TIMEOUT)
            .then(
                ...
                function(e){selena.regActionResult("Клик на пункте Edit. Редактирование сообщения " + chatMessageBefore + " не активировалось.", "\n", e.message, 0)}
            )
    ...
    .keys(["Enter"])
        .waitForVisible("//*[@contenteditable='true' and contains(text(),'" + chatMessageBefore + "')]", TIMEOUT, true)
            .then(
                ...
                function(e){selena.regActionResult("Ввод " + chatMessageAfter + ". Сообщение " + chatMessageBefore + chatMessageAfter + " не сохранилось.", "\n", e.message, 0)}
            )
    ...
    .taskCardOpen(taskName)
        .waitForVisible("//*[@role='messageHistory' and contains(text(),'" + chatMessageBefore + chatMessageAfter + "')]", TIMEOUT)
            .then(
                ...
                function(e){selena.regActionResult("Проверка во втором табе: Отредактированное сообщение " + chatMessageBefore + chatMessageAfter + " не сохранилось.", "\n", e.message, 0)}
            )
    .keys(["Escape"])
    ...
    .keys(["Enter"])
        .waitForVisible("//*[@role='messageHistory' and contains(text(),'" + chatMessage + "')]", TIMEOUT)
            .then(
                ...
                function(e){selena.regActionResult("Сообщение " + chatMessage + " не появилось в истории карточки " + taskName + "", "\n", e.message, 0)}
            )
    ...
    .click("//*[@role='dropdown']//*[@title='New card']")
        .waitForVisible("//*[@role='cardTitle' and contains(text(),'" + chatMessage + "')]", TIMEOUT)
            .then(
                ...
                function(e){selena.regActionResult("Клик на пункте New Card. Новой карточка с тайтлом " + chatMessage + " не создалась.", "\n", e.message, 0)}
            )
    .keys(["Escape"])
    ...
    .taskCardOpen(taskName)
        .waitForVisible("//*[@role='title' and contains(text(),'" + chatMessage + "')]", TIMEOUT)
            .then(
                ...
                function(e){selena.regActionResult("Проверка во втором табе: Карточка " + chatMessage + " в чате родителя " + taskName + " не найдена.", "\n", e.message, 0)}
            )
    .keys(["Escape"])
    ...
    .taskCardOpen(taskName)
        .waitForVisible("[class*='deleted']", TIMEOUT)
            .then(
                ...
                function(e){selena.regActionResult("Проверка по втором окне. Сообщение " + chatMessage + " не удалено.", "\n", e.message + "\n" + e.stack)}
            )
    .keys(["Escape"])
